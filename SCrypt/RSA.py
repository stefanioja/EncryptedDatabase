...
def RSA_encrypt(plaintext: bytes, public_key: tuple[int, int]) -> bytes:
    ...
    m = int.from_bytes(pad(plaintext, get_size_in_bytes(n)),
...
def RSA_encrypt_file(path_src: str, path_dest: str, public_key: tuple[int, int]) -> None:
    ...
    with open(path_src, 'rb') as src, open(path_dest, 'wb+') as dest:
        block_walk(src, dest, block_size, lambda x: RSA_encrypt(x, public_key))
...
def RSA_decrypt_file(path_src: str, path_dest: str, private_key: tuple[int, int]) -> None:
    ...
    def decrypt_func(x): return RSA_decrypt(x, private_key)
    with open(path_src, 'rb') as src:
        if path_dest is not None:
            with open(path_dest, 'wb+') as dest:
                block_walk(src, dest, block_size, decrypt_func)
        else:
            block_walk(src, sys.stdout.buffer, block_size, decrypt_func)
